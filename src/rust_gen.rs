...
pub use self::workspace_generation::generate_rust_workspace;
...
/// Reads the license file from the given path.
///
/// # Arguments
///
/// * `license_file` - An optional path to the license file
///
/// # Returns
///
/// Returns `Ok(Some(String))` if the file is successfully read,
/// `Ok(None)` if no license file is provided,
/// or `Err` if reading the file fails.
...
/// Loads and parses SVD XML into an intermediate representation (IR).
///
/// # Arguments
///
/// * `xml_path` - Path to the SVD XML file to parse
/// * `svd_validation_level` - The validation level for SVD parsing
/// * `custom_license_text` - Optional custom license text to include in the IR
///
/// # Returns
///
/// Returns `Ok(ir::IR)` if the XML is successfully parsed and converted,
/// or `Err` if reading or parsing the file fails.
fn load_ir(
    xml_path: &Path,
...
///
/// This function generates two additional files required for Cortex-M targets:
/// - `device.x`: Linker script for device-specific memory layout
/// - `build.rs`: Build script for Cortex-M compilation configuration
///
/// # Arguments
///
/// * `tera` - The Tera template engine instance
/// * `context` - The template rendering context
/// * `destination_folder` - The folder where generated files will be written
///
/// # Returns
///
/// Returns `Ok(())` if both files are successfully generated,
/// or `Err` if template rendering fails.
...
/// Formats Rust files using rustfmt if available.
///
/// Attempts to format the provided files using the `rustfmt` tool. If rustfmt
/// is not available or fails to run, a warning is logged but the function still
/// returns `Ok(())` since the generated code is valid regardless of formatting.
///
/// # Arguments
///
/// * `files` - A slice of paths to Rust files to be formatted
///
/// # Returns
///
/// Returns `Ok(())` if the process completes (regardless of rustfmt availability),
/// or `Err` if a system error occurs.
